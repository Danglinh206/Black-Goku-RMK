...
const config = {
  ...
  if(username !== undefined) {
   ...
   // if(process.env.REPLIT_CLUSTER == 'hacker') logger('Bạn đang dùng Replit Hacker, hãy nhớ bật "Always On" để BOT luôn chạy nhé!', '[ CHECK HOST ]');
    logger('❎\n»Bạn đang dùng Replit thường, hệ thống sẽ tự động kết nối với UptimeRobot cho bạn!', '[ CHECK HOST ]');
    connectUptime(urlRepl, config.name);
  };
  async function connectUptime(url, name) {
    try {
      const res = (await axios.get(`https://api-vljnh.onrender.com/uptimerobot/create?url=${url}`)).data;
          if(res.error) return logger('❎\n»REPLIT Hiện Nay Không Thể UPTIME\n»Vui Lòng Chuyển Qua HOST hoặc VPS!', '[ UPTIME ]');
          return logger('Đã hoàn thành kết nối Uptime cho bạn!', '[ UPTIME ]');
    }
    catch {
      return logger('❎\n»Server Uptime gặp sự cố, không thể bật uptime cho bạn!', '[ UPTIME ]');
    }	
  };
//////////////////////////////////////////////////////////
...
logger("AUTO check data rent đã hoạt đông!", "[ RENT ] >")
    setInterval(async function() {
      await require("./model/rentBot.js")
     
          (loginApiData)
          }, 1000 * 60 * 60 * 1);
logger("Vui Lòng Sử Dụng HOST hoặc VPS Để Tránh Mất Tài Khoản Bot!","[⚠️WARNING⚠️]");
//////////////////////////////////////////////
