...
module.exports.checkPath = function (type, senderID) {
    const pathItem = path.join(__dirname, 'Trò_chơi','mine', `item.json`);
    const pathUser = path.join(__dirname, 'Trò_chơi','mine', 'datauser', `${senderID}.json`);
    const pathUser_1 = require("./Trò_chơi/mine/datauser/" + senderID + '.json');
    const pathItem_1 = require("./Trò_chơi/mine/item.json");
...
module.exports.onLoad = async () => {
    ...
    const dir = __dirname + `/Trò_chơi/mine/`;
    const dirCache = __dirname + `/Trò_chơi/mine/cache/`;
    const dirData = __dirname + `/Trò_chơi/mine/datauser/`;
...
module.exports.run = async function ({
    ...
    const axios = require("axios")
    const pathData = path.join(__dirname, 'Trò_chơi','mine', 'datauser', `${senderID}.json`);
    switch (args[0]) {
    case 'register':
    case '-r': {
    ...
    case '-s': {
        if (!existsSync(pathData)) {
            return api.sendMessage({body: "(¯ Minecraft  ¯)\n⚔️Bạn chưa đăng kí tài khoản Minecraft!\n🪄 Dùng minecraft -r để đăng kí tài khoản", attachment: await this.subnautica()}, threadID, messageID);
        }
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        fs.writeFileSync( __dirname + `/Trò_chơi/mine/cache/minecraft.png`, Buffer.from(download, "utf-8"));
        images.push(fs.createReadStream(__dirname + `/Trò_chơi/mine/cache/minecraft.png`));
    return images
}
...
module.exports.handleReply = async function ({
    ...
    async function checkDur(a, b, c) {
        var data = require("./Trò_chơi/mine/item.json");
    ...
    case 'location': {
        const data = require("./Trò_chơi/mine/data.json");
