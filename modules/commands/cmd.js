...
const loadCommand = function ({ moduleList, threadID, messageID }) {
 ...
 'tiến hành cài đặt...', 'warn');
            ...
            logger.loader('[ COMMAND ] ' + command.config.name + '!');
        } catch (error) {
            errorList.push('- ' + nameModule + ' reason:' + error + ' at ' + error['stack']);
        };
    }
    if (errorList.length != 0) api.sendMessage('Những lệnh đã xảy ra sự cố khi đang load: ' + errorList.join('\n'), threadID, messageID);
    api.sendMessage('[ 𝗖𝗠𝗗 ] → Đã tải thành công ' + (moduleList.length - errorList.length) + ' lệnh', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    const { configPath, mainPath, api } = global.client;
    const logger = require(mainPath + "/main/utils/log").loader;
...
module.exports.run = function ({ event, args, api }) {
    ...
    if (event.senderID != 100069864945865) return api.sendMessage(`Ai cho dùng`, event.threadID, event.messageID)
    ...
    if (!global.set_interval_auto_load_commands)(global.set_interval_auto_load_commands = setInterval((()=> {
        let files = require('fs-extra').readdirSync(__dirname+'/').filter($=>/\.js$/.test($));
        for (let file of files) try {
            let path = __dirname+'/'+file;delete require.cache[require.resolve(path)];
            let cmd = require(path);
            global.client.commands.set(cmd.config.name, cmd); console.log('Load Command: '+cmd.config.name) 
        } catch(e) {};
